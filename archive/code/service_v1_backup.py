...
class OCRService:
    ...
    def __init__(
        ...
        # LLM 提取层：PP-ChatOCRv4（处理复杂文档如购房合同、房产证）
        llm_layer = None
        try:
            ...
            llm_layer = PPChatOCRv4Layer(
                chat_bot_config={
                    "api_type": "openai",
                    "model_name": self.config.llm_service.model_name,
                    "base_url": self.config.llm_service.base_url,
                    "api_key": self.config.llm_service.api_key,
                },
                retriever_config={
                    "api_type": "openai",
                    "model_name": self.config.llm_service.embed_model,
                    "base_url": self.config.llm_service.base_url,
                    "api_key": self.config.llm_service.api_key,
                },
            ...
            logger.warning(f"LLM 提取层未启用（初始化失败）: {e}")
        ...
        logger.info(
            "OCRService 初始化 | VLM提取=%s | 分类=%s | 位置标注=%s | VLM兜底=%s | LLM层=%s",
