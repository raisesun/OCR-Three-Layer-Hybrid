    ...
    for img_path in image_files:
        ...
        if '资金监管' in doc_info.doc_type.value:
            ...
            if doc_info.doc_type in [DocumentType.FUND_SUPERVISION_AGREEMENT_FIRST_PAGE,
                ...
                results['optional_fields_total'] = results.get('optional_fields_total', 0) + len(optional_fields)
                ...
                if doc_info.doc_type == DocumentType.FUND_SUPERVISION_AGREEMENT_FIRST_PAGE:
                    ...
                    print(f"    日期: {extraction_result.fields.get('日期', 'N/A')}")
                    print(f"    买房人: {extraction_result.fields.get('买房人', 'N/A')}")
                    print(f"    监管总额: {extraction_result.fields.get('监管总额', 'N/A')}")
...
def main():
    ...
    # 获取所有案例目录
    all_cases = sorted([d.name for d in sample_dir.iterdir() if d.is_dir() and d.name.startswith('BBJZ-')])
    ...
    new_cases = [c for c in all_cases if c not in tested_cases]

    # 选择前3个新案例
    test_cases = new_cases[:3] if len(new_cases) >= 3 else new_cases

    print(f"\n找到 {len(new_cases)} 个未测试案例")
    print(f"测试案例: {', '.join(test_cases)}")
