...
class RuleExtractionLayer(IExtractionLayer):
    ...
    def extract(self, doc_info: DocumentInfo, key_list: List[str]) -> ExtractionResult:
        ...
        try:
            ...
            if (
                ...
                fields = self._extract_household_register(
                ...
                fields = self._extract_fund_supervision(
    ...
    def _extract_id_card_with_labels(
    ...
    def _extract_id_card_without_labels(
        self, full_text: str, key_list: List[str], fields: Dict[str, str]
    ):
    ...
    def _extract_marriage_certificate(
    ...
    def _extract_divorce_certificate(
        self, full_text: str, key_list: List[str]
    ) -> Dict[str, str]:
        """
        从离婚证文本中提取字段
    ...
    def _extract_id_card_back(
        ...
        if "有效期限" in key_list:
            ...
            if match:
                ...
                match = re.search(
                    r"(\d{4}\.\d{2}\.\d{2}-\d{4}\.\d{2}\.\d{2})", full_text
                )
                if match:
                    fields["有效期限"] = match.group(1)
    ...
    def _extract_property_certificate(
        self, full_text: str, key_list: List[str]
    ) -> Dict[str, str]:
        """从房产证文本中提取字段"""
        fields = {}
    ...
    def _extract_property_certificate_content(
        self, full_text: str, key_list: List[str]
    ) -> Dict[str, str]:
        """
    ...
    def _extract_fund_supervision_certificate(
        self, full_text: str, key_list: List[str]
    ) -> Dict[str, str]:
        """从资金监管凭证文本中提取字段
    ...
    def _extract_divorce_agreement(
        self, full_text: str, key_list: List[str]
    ) -> Dict[str, str]:
        """从离婚协议文本中提取字段"""
        fields = {}
